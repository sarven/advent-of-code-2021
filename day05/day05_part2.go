...
func (l Line) getMinXPoint() Point {
	if l.p1.x > l.p2.x {
		return l.p2
	}
...
func (l Line) getMaxXPoint() Point {
	if l.p1.x > l.p2.x {
		return l.p1
	}
...
func markIntersection(iPoints [MAX][MAX]int, line Line) [MAX][MAX]int {
	...
	if 0 != line.p1.x-line.p2.x {
		if 1 == (maxY-minY)/(maxX-minX) {
			for i, j := line.getMinXPoint().x, line.getMinXPoint().y; i <= line.getMaxXPoint().x; i++ {
				iPoints[j][i]++

				if line.getMinXPoint().y > line.getMaxXPoint().y {
					j--
				} else {
					j++
				}
